...
Version tags (see `VERSION`): **v2.21.0** = `3586e6c83d4e70fc3bdabf048090a924fc2c8250`,
...
| --- | --- | --- |
| 2026-08-11 | `3586e6c83d4e70fc3bdabf048090a924fc2c8250` | **v2.21.0** — the quarterback throws properly and the passes are catchable: the Throw clip re-authored against measured collegiate-QB kinematics and checked with a new `tools/measure-clip.mjs`, and the launch-angle solve fixed (it used flat-ground range while the ball falls from hand height to catch height, so every pass sailed 2.8yd long and the completion rate was 0.3%). Plus: the camera now sits behind whoever has the ball rather than behind the user; a defender who pulls a flag reaches for it and rips it, and the carrier's flag actually comes off (`flagPulled` was never once set to true); the field is the regulation 70 x 30, out of bounds is decided by crossing the line instead of coming within 0.4yd of it, a pass can leave the field, and it is three downs to score once you have crossed midfield |
